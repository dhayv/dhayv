...
### [WiseWalletWin – Production Cloud Platform](https://github.com/dhayv/WiseWalletWin)
Operated production infrastructure for 18 months through 6 architecture migrations—real users, real constraints, real decisions.

**The migrations:**
1. Monolithic EC2 → hit CPU ceiling
2. Distributed architecture with S3/CloudFront → needed better data layer
3. SQLite → MongoDB Atlas → 99.999% uptime after data loss incident
4. Monolith → Containerized Lambda → cut cold starts 70%
5. Evaluated ECS, Lightsail, Amplify → rejected for cost/complexity
6. Lambda → App Runner → eliminated cold start problem entirely

**Results:**
- Deployments: 30 min → 5 min (CI/CD with GitHub Actions OIDC)
- Infrastructure costs: down 40%
- Container footprint: reduced 60% via multi-stage builds
- All under $20/month budget ceiling
